...
func Test_First_Day(t *testing.T) {
	...
	assert.Equal(t, items[8].Quality, 4)
}

func Test_Aged_Brie_30_Days(t *testing.T) {
	var items = []*gildedrose.Item{
		{"Aged Brie", 2, 0},
	}
	...
	assert.Equal(t, items[0].SellIn, -27)
	assert.Equal(t, items[0].Quality, 50)
}

func Test_Backstage_30_Days(t *testing.T) {
	var items = []*gildedrose.Item{
		{"Backstage passes to a TAFKAL80ETC concert", 15, 20},
		{"Backstage passes to a TAFKAL80ETC concert", 10, 49},
		{"Backstage passes to a TAFKAL80ETC concert", 5, 49},
	...
	assert.Equal(t, items[0].SellIn, -14)
	assert.Equal(t, items[0].Quality, 0)

	assert.Equal(t, items[1].SellIn, -19)
	assert.Equal(t, items[1].Quality, 0)

	assert.Equal(t, items[2].SellIn, -24)
	assert.Equal(t, items[2].Quality, 0)
}

func Test_Sulfuras_30_Days(t *testing.T) {
	var items = []*gildedrose.Item{
		{"Sulfuras, Hand of Ragnaros", 0, 80},
		{"Sulfuras, Hand of Ragnaros", -1, 80},
	}
	...
	assert.Equal(t, items[1].SellIn, -1)
	assert.Equal(t, items[1].Quality, 80)
}

func Test_Conjured_30_Days(t *testing.T) {
	var items = []*gildedrose.Item{
		{"Conjured Mana Cake", 3, 6}, // <-- :O
	}
	...
	assert.Equal(t, items[0].SellIn, -26)
	assert.Equal(t, items[0].Quality, 0)
}
